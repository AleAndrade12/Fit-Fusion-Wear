...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_producto'])) {
    ...
    $precio = isset($_POST['precio_venta']) ? mysqli_real_escape_string($conn, $_POST['precio_venta']) : '';
    $categoria = isset($_POST['categoria']) ? mysqli_real_escape_string($conn, $_POST['categoria']) : '';
    $stock = isset($_POST['stock']) ? mysqli_real_escape_string($conn, $_POST['stock']) : '';
...
<head>
    ...
    <script>
        // Función para habilitar/deshabilitar los campos de entrada
        function toggleEdit() {
            var inputs = document.querySelectorAll('input, select, textarea');
            inputs.forEach(function(input) {
                input.disabled = !input.disabled;  // Alternar entre habilitar y deshabilitar
            });
            document.getElementById('editar-btn').style.display = 'none';  // Ocultar el botón de editar después de presionarlo
            document.getElementById('guardar-btn').style.display = 'block';  // Mostrar el botón de guardar
        }
    </script>
</head>
<body>
    ...
    <!-- Incluir la barra de búsqueda desde buscarProducto.php -->
    <?php include('buscarProducto.php'); ?>
        ...
        <form method="POST" action="" enctype="multipart/form-data">
            <div class="form-table-style">
                ...
                <div class="form-columna">
                    ...
                    <label>Stock:</label>
                    <input type="number" name="stock" value="<?= isset($producto) ? $producto['stock'] : '' ?>" placeholder="Stock" required disabled>

                    <div class="form-botones">
                        <button type="button" id="editar-btn" onclick="toggleEdit()">Editar</button>
                        <button type="submit" name="guardar_producto" id="guardar-btn" style="display:none;">Guardar</button>
                        <a href="eliminarProducto.php?codigo=<?= $producto['codigo'] ?>" onclick="return confirm('¿Estás seguro de eliminar este producto?');">
